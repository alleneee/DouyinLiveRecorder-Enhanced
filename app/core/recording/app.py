...
class RecordingApplication:
    def __init__(
        ...
        *,
        ...
        logger.info("正在初始化录制应用")
        logger.debug("配置: 文件监控={}, CLI={}, 间隔={}秒", 
                    enable_file_watcher, enable_cli, file_watch_interval)
        ...
        snapshot = self._service.bootstrap()
        logger.info("已加载 {} 个房间", len(snapshot.rooms))
    ...
    def start(self) -> None:
        logger.info("正在启动录制应用")
        
        self._supervisor.start()
        logger.info("录制调度器已启动")
        ...
        if self._cli:
            self._cli.start()
            logger.info("CLI 接口已启动")
        
        logger.info("录制应用启动完成")

    def stop(self) -> None:
        logger.info("正在停止录制应用")
        ...
        if self._cli:
            self._cli.stop()
            logger.info("CLI 接口已停止")
        
        self._supervisor.stop()
        logger.info("录制调度器已停止")
        
        logger.info("录制应用已完全停止")

    ...
    def _recording_worker(self, session_id: str, room_id: int):
        ...
        try:
            ...
            if not stream_info['is_live'] or not stream_info['stream_url']:
                logger.warning(f"直播未开播或获取流地址失败: room_id={room_id}")
            ...
            try:
                while room.live_status == LiveStatus.LIVE:
                    ...
                    filename = f"{timestamp}_seg{segment_index}.{settings.video_save_type.lower()}"
                    ...
                    ffmpeg_cmd = [
                        ...
                        '-f', settings.video_save_type.lower(),
                        file_path
                    ]
